...
assn_r2 = DA(students, schools_r2, capacities, verbose=true)  # Compute r2 assignments

for i in [assn_r1, assn_r2]                         # Compare the assignments. r1 should dominate rankwise.
    println(i)
end

all(assn_r1[2] .<= assn_r2[2]) ? println("✓ Rankwise dominance holds") : println(assn_r2[2] - assn_r1[2])